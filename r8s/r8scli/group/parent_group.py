...
def describe(parent_id: str, group_id: str):
    ...
    return init_configuration().resource_group_get(
        parent_id=parent_id,
        group_id=group_id,
    )
...
@click.option('--allowed_tag', '-at', multiple=True, required=False,
              help='List of tag keys to add')
@click.option('--group_arn', '-arn', multiple=True, required=False,
...
@cli_response()
def describe(parent_id: str, allowed_tag, group_arn, scale_step, cooldown_days):
    """
    Describes a R8s parent resource group configuration
    """
    from r8scli.service.initializer import init_configuration
...
@click.option('--tag', '-t', multiple=True, required=True,
              help='List of tag keys to add')
@cli_response()
...
@click.option('--group_id', '-gid', type=str, required=True,
              help='Resource group id to update')
@click.option('--group_arn', '-arn', multiple=True, required=True,
