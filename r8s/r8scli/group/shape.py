...
@click.option('--network_throughput', '-nt', type=float, required=True,
              help='Shape network throughput in mbps.')
...
@cli_response()
def add(name, cloud, cpu, memory, network_throughput, iops,
    ...
    return init_configuration().shape_post(
        ...
        network_throughtput=network_throughput, iops=iops,
...
@click.option('--memory', '-m', type=float, required=False,
              help='Shape memory (in gb).')
@click.option('--network_throughput', '-nt', type=float, required=False,
              help='Shape network throughput in mbps.')
...
@click.option('--architecture', '-arch', type=str, required=False,
              help='Shape architecture.')
@cli_response()
def update(name, cloud, cpu, memory, network_throughput, iops,
           family_type, physical_processor, architecture):
    """
    Updates a R8s Shape.
    """
    ...
    return init_configuration().shape_patch(
        name=name, cloud=cloud, cpu=cpu, memory=memory,
        network_throughput=network_throughput, iops=iops,
        family_type=family_type, physical_processor=physical_processor,
        architecture=architecture)
